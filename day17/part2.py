...
def main(registers: list[Register], program: Program) -> None:
    def evaluate(registers: list[Register], program: Program) -> list[int]:
        out = []
        pointer = 0
    ...
    program_instructions = [
        instruction.expand() for instruction in program.instructions
    ]
    flattened = [item for sublist in program_instructions for item in sublist]

    candidate = [0]
    for length in range(1, len(flattened) + 1):
        out = []

        for n in candidate:
            for offset in range(2**3):
                a_r = (2**3) * n + offset
                registers[0].value = a_r

                if evaluate(registers, program) == flattened[-length:]:
                    out.append(a_r)
